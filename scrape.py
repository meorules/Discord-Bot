...
while count <= maxOffsetNoBrazilians:
    ...
    for row in rows:
        ...
        playerUrl = domainUrl + cols[1].find('a').get('href')
    ...
    cur.executemany("INSERT INTO Players(PlayerName,Position,Age,Rating,Team,Height,Weight,Crossing,Finishing,Heading,Jumping,Penalties,WeakFoot,SkillMoves,Passing,Defending,Attacking,Country,PlayerUrl) VALUES(?, ?, ?,?, ?, ?,?, ?, ?,?, ?, ?,?, ?, ?,?, ?, ?,?)", data)
    con.commit()
    count+= 60